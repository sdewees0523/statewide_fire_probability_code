...
# Reading in shapefile with all the independent variables 
...
# Reading in cultivated raster to use as template for future rasterization
...
for (year_i in years) {
  ...
  # Use model to predict fire probability for year of interest
  ...
  # Turning predicted fire probability shapefile into a raster, using cultivated raster as template
  ...
  write_stars(pred_raster, here("02_WRF", 
                                ...
                                paste0("yearly_noffwi_noveg_model_prediction_", year_i, ".tif")))
}
