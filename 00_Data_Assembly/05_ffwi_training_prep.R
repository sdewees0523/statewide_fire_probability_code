...
# Looping through the years of interest to make shapefile for model training
for (years in 2003:2016) {
  ...
  california <- c(time_stable, # Making raster list of all independent raster variables
    ...
    filter(diskTest_r2_water.tif == 1) %>% # Filtering shapefile to poisson disk sampled points
    drop_na() %>% # Dropping any rows with NA values that will be excluded from gam model anyways
  ...
  california_list[[year_label]] <- california #Adding current years data to list created just before for loop
}
...
#Writing to disk
st_write(california_all_spatial, here("data", "Assembled_Data", "california_all_training_disk.shp"), append = FALSE)
...
write.csv(california_all, here("data", "ffwi_training_disk.csv"), row.names = F, append = FALSE)
